<template>
  <label
    ...
    class="drag__input"
    :class="{ 'drag__input--dragged': isDragged || Boolean(value?.length) }"
    for="redacao"
  ...
  >
    <app-button
      size="lg"
      class="drag__input-event px-md-15"
      @click.passive="handleClickLabelChangePhoto"
    >
      <app-text as="strong" color="white" weight="semibold">Selecione uma imagem</app-text>
      <ph-image class="ms-2" size="24" :color="primary" />
    </app-button>

    <app-text class="separator mt-5 drag__input-no-event" size="sm" as="span" color="text-400">
      Ou
    </app-text>

    <div class="d-flex align-center mt-5 drag__input-no-event">
    ...
    <footer class="drag__input-footer d-flex align-center">
