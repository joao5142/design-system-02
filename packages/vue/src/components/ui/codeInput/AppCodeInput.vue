...
function validateBackOrNextKeyboard(event: KeyboardEvent) {
  ...
  if (event.target) {
    ...
    if (event.key === 'Backspace') {
      const previousSiblingEl = target?.previousElementSibling as HTMLElement | null;

      if (previousSiblingEl instanceof HTMLElement) {
        previousSiblingEl.focus();
        target.value = '';
      }
    }
  }
...
.input-number {
  ...
  @container code (max-width: 430px) {
    & {
  ...
  @container code (max-width: 360px) {
    & {
  ...
  @container code (min-width: 900px) and (max-width: 1100px) {
    & {
      width: 36px !important;
      height: 36px !important;
    }
